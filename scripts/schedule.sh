...
LABEL_UPDATE="com.gallant.plaud-meetings-digest.auto-update"
...
PLIST_UPDATE="$HOME/Library/LaunchAgents/$LABEL_UPDATE.plist"
...
# Bundle prefix — where the heartbeat wrapper + auto-update live.
# Defaults to script's own dir parent (when invoked from unpacked source),
# falls back to ~/Library/Application Support/plaud-meetings-digest (where
# bootstrap drops the bundle).
...
AUTO_UPDATE_SCRIPT="$BUNDLE_PREFIX/scripts/auto-update.sh"
...
try:
    ...
    print("AUTO_UPDATE=" + (checks.get("auto_update") or ""))
except Exception:
    pass
PYEOF
)
  while IFS='=' read -r key value; do
    case "$key" in
      ...
      AUTO_UPDATE) CHECK_AUTO_UPDATE="$value" ;;
    esac
  done <<< "$HB_CONFIG"
fi

# ---- Unload current + legacy --------------------------------------------
for L in "$LABEL_LUNCH" "$LABEL_EOD" "$LABEL_ROLLUP" "$LABEL_UPDATE" "${LEGACY_LABELS[@]}"; do
...
# If a CheckId is set + the heartbeat wrapper is on disk, the plist invokes
# bash run-with-heartbeat.sh --check-id X --ping-base-url Y -- <inner cmd>.
# Otherwise it invokes the inner command directly.
write_plist() {
  local label="$1"
  local plist_path="$2"
  local check_id="$3"
...
if [[ -f "$AUTO_UPDATE_SCRIPT" ]]; then
  write_plist "$LABEL_UPDATE" "$PLIST_UPDATE" "$CHECK_AUTO_UPDATE" \
    "$(daily_calendar 3 0)" \
    "/bin/bash" "$AUTO_UPDATE_SCRIPT" "--config-path" "$CONFIG_PATH"
fi
...
if [[ -f "$PLIST_UPDATE" ]]; then
  ...
  warn "Auto-update plist NOT installed — auto-update.sh missing at $AUTO_UPDATE_SCRIPT"
fi
printf "  Logs:         %s\n" "$LOG_OUT"
printf "\n"
