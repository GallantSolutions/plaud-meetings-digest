...
- **v2.2.5** — Closure mechanism. Adds a fifth column `Done?` to the weekly rollup's carry-over table with a `[ ]` checkbox per row, plus a Word-hidden 8-char `id:XXXXXXXX` text run in the Action cell so closure parsing can correlate `[x]` marks back to JSONL rows even if the client lightly edits action wording. New `state_store.append_closures()` / `load_closed_ids()` + `closures.jsonl` persistence; `state_store.read_items()` now defaults `exclude_closed=True` so closed items disappear from subsequent rollups automatically. New `rollup_docx_writer.harvest_closures(meeting_root)` walks recent KPR*.docx files for `[x]` marks; the weekly-rollup skill calls it BEFORE generating the new rollup so user-side closures are baked in. `ActionItem` dataclass gains `item_id` (auto-derived SHA1 prefix of action + source_file_id) with legacy backfill on read. Solves the actual closure landscape — most action items close OUTSIDE meetings (hallway handoffs, async deliverables, John's own work between meetings); the rollup becomes the single closure surface, exercised once per week, ~60 seconds. CLIENT-USER-GUIDE bumped to edition 1.1 with a third user-side habit, a Saturday closure ritual, an updated rollup-anatomy section, and a new quick-reference card line. Liberal closure-token matching (`[x]` / `[X]` / `☒` / `☑` / `✓` / `✔`) accommodates Word autocorrect.
- **v2.2.4** — Professional pharma-corporate document styling. Stripped ALL emoji from the rendered Word documents (rollup + per-meeting) and the client-facing user guide. Replaced casual symbols with text labels: aging markers became `CRITICAL` / `ATTENTION` / `OPEN` text labels in deep red / deep amber / graphite. Section headers are now ALL CAPS in deep navy with subtle bottom rules (matches what a pharma exec expects from a board-grade deliverable). Carry-overs in the weekly rollup now render as a clean 4-column table (`Action Item | Days Open | Status | Source Meeting`) instead of bulleted aging markers. Per-meeting documents gain a top-of-doc `Recap` section (2-3 sentence executive summary written by Claude during extraction). Action item checkboxes changed from `☐` Unicode ballot box to plain `[ ]` text — universally readable, prints cleanly, and the client marks completion by editing to `[x]` in Word. CLIENT-USER-GUIDE.md rewritten in professional tone (no casual language, no emoji). Brand colors locked at `#1A2A44` (deep navy), `#3D4A59` (graphite), `#A81F1F` (deep red for critical), `#B86E18` (deep amber for attention). Base typography Calibri 11pt to match Word default. No emoji touches a client surface again, ever.
- **v2.2.3** — Two operator-requested improvements: (1) Friday rollup now opens with `⏱️ Still open from prior weeks — action needed` at the TOP of the doc instead of buried below new items + by-context. Stuck commitments are the highest-leverage info in the rollup; he should see them before planning next week's focus. Zero-stuck weeks render `✅ Clean slate` as a positive signal. (2) Auto-rollback safety net: `auto-update.{ps1,sh}` writes a `.last-update.json` marker after each successful update; the heartbeat wrapper checks it when a scheduled job fails non-zero and, if the update happened within the last 24h, restores the previous version from the snapshot at `.versions/<from_version>/` + re-registers tasks + pings heartbeat with a 'fail' marker so the operator gets alerted. One-shot per update (sets `rolled_back: true` so it doesn't recurse). Defenses the single highest-impact failure mode that's hardest for the client to recover from manually.
